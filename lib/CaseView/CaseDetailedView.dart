...
class CaseDetailedView extends StatefulWidget {
  ...
  final tabValue;
  final bool isSearch;
  CaseDetailedView(this.caseId, { this.isSearch,this.tabValue});
...
String commentTypeFinalSelected;
 ...
 class _CaseDetailedView extends State<CaseDetailedView> with SingleTickerProviderStateMixin , AutomaticKeepAliveClientMixin <CaseDetailedView>{
   @override
 ...
 int initialIndexTab = 0;
  ...
  Future<void> getIncidentFullView() async{
    ...
    CustomResponse response = await ApiCall.makeGetRequestToken('incident/get?id=${widget.caseId}&issearch=${widget.isSearch?? false}'
  ...
  Widget noConnectionMessage(BuildContext context){
        ...
        child: Center(
          child: Column(
            ...
            children: <Widget>[
              ...
              FloatingActionButton.extended(onPressed: (){getIncidentFullView();
                  if (widget.tabValue != null)
                     initialIndexTab = widget.tabValue;},
              ...
              Row(
                ...
                children: <Widget>[
                  Icon(Icons.star, size: 10.0,color: Colors.red,),
                  SizedBox(width: 5.0,),
                  Text("The Cases will be Uploaded only when you are connected to a Network", style: TextStyle(fontSize: 11.5), maxLines: 2,
   ...
   Future<bool> _onWillPop() async{
    allComments.clear(); cacheChatList.clear(); chatList.clear();
        ...
        return DefaultTabController(
          ...
          child: Scaffold(
            appBar: AppBar(
              ...
              actions: dataLoading || !isConnected  ? <Widget>[] :<Widget>[
                IconButton(
                  iconSize: 20,
                  ...
                  icon: Icon(Icons.remove_red_eye,
                      color: state.isInWatchList ? Colors.teal : Colors.black),
                ),
                IconButton(
                  iconSize: 20,
                  tooltip: "Share this Case",
                  onPressed: () async {
                    var link = await CommonFunction.createDynamicLink(caseId: "${widget.caseId}",
                  ...
                  onSelected: (value) {
                    if (value == 0 && isConnected)
                      Navigator.push(context, MaterialPageRoute(
                          builder: (context) =>
                              ReportCase(widget.caseId, caseDetails)));
                  },
                )
              ],
              bottom: TabBar(
                ...
                tabs: <Tab>[
                  Tab(child: Text(
                    'Summary', style: TextStyle(color: Colors.black),
                    maxLines: 1,
                    overflow: TextOverflow.ellipsis,)),
                  Tab(child: Text(
                    'Discussion', style: TextStyle(color: Colors.black,),
                    maxLines: 1,
                    overflow: TextOverflow.ellipsis,)),
                  Tab(child: Text(
              ...
              title: Text(dataLoading ? "" :
              '${caseDetails["caseidentifier"] == null?
                caseDetails["caseid"].toString(): caseDetails["caseidentifier"].toString()}',
                style: TextStyle(color: Colors.black, ),),
            ),
            body: !isConnected ? noConnectionMessage(context) :!dataLoading
                ? TabBarView(
                  physics: NeverScrollableScrollPhysics(),
                 controller: _tabBar,

                  children: [
                    CaseView(navigateController,caseDetails, key: PageStorageKey('summary'),),
                    CaseDiscussion(caseDetails, token, key: PageStorageKey('discussion'),),
                    CaseGallery(caseDetails, key: PageStorageKey('gallery'),),
              ],
            ) : authError ? Container()
        ...
        child: Column(
          ...
          children: <Widget>[
            ...
            Wrap(
              ...
              children: <Widget>[
                Text(dateFormatter.format(DateTime.parse(widget.caseDetails["createddate"])), style: dateStyle,),
                SizedBox(width: 5.0,),
                Text(  widget.caseDetails["isanonymous"] ? "Initiated by Anonymous" : "Initiated by ${widget.caseDetails["addedby"]["name"]}" , style: highLightName,),
                SizedBox(width: 5.0,),
                Text(  "(${widget.caseDetails["photos"].length})", style: highLightName, maxLines: 2,overflow: TextOverflow.ellipsis,),
              ],
            ),
            SizedBox(height: 5.0,),
            AnimationLimiter(
                      ...
                      child: ScaleAnimation(
                        ...
                        child: Container(
                            ...
                            child: ClipRRect(
                                borderRadius: BorderRadius.all(Radius.circular(5.0)),
                                child:
                                Image(image: NetworkImage(ApiCall.imageUrl + widget.caseDetails["photos"][index]["photo"]),fit: BoxFit.cover,
                        ...
                        child: FadeInAnimation(
                          child:   InkWell(
                            onTap: () => Navigator.push(context, MaterialPageRoute(
                                builder: (context) => GalleryImageView(widget
                                    .caseDetails["updates"][i]["photos"], index, widget.caseDetails["updates"][i]))),
                            child: Container(
                                padding: const EdgeInsets.all(2.0),
                                decoration: BoxDecoration(
                                  border: Border.all(color: Colors.blueGrey),
                                ...
                                child: ClipRRect(
                                  ...
                                  Image(image: NetworkImage(ApiCall.imageUrl + widget.caseDetails["updates"][i]["photos"][index]["photo"]),
                                    fit: BoxFit.cover,
                                    loadingBuilder:(BuildContext context, Widget child,ImageChunkEvent loadingProgress) {
                                      if (loadingProgress == null) return child;
                                      return Center(
